...
*	Assume student status for loginID.
*/
import java.sql.*;
import java.util.*;
import java.io.*;
...
public class Project_DB_Publications_student {
	...
	public Project_DB_Publications_student(String loginID)
	{
	...
	*/
	...
	public void init_UI()
	{
		...
		try {
			...
			ResultSet rs_sbookborrow_due = stmt.executeQuery("SELECT * FROM sBookBorrow WHERE etime < to_date('" + sdf.format(curDate) + "', 'mm/dd/yy hh24:mi')");
			ArrayList<Integer> show_sb_bid = new ArrayList<Integer>();
			while (rs_sbookborrow_due.next()) 
			{
				show_sb_bid.add(rs_sbookborrow_due.getInt("sb_bid"));
			}
			rs_sbookborrow_due.close();
			for (int i=0; i<show_sb_bid.size(); i++)
			{
				stmt.executeUpdate("UPDATE sBookBorrow SET otmark = 1 WHERE sb_bid = " + show_sb_bid.get(i));
			}
			// check if it pass 90 days over due
			Date before90Days = new Date(curDate.getTime() - 90*DAY_IN_MS);
			rs_sbookborrow_due = stmt.executeQuery("SELECT * FROM sBookBorrow WHERE etime < to_date('" + sdf.format(before90Days) + "', 'mm/dd/yy hh24:mi')");
			HashSet<String> show_sid = new HashSet<String>();
			while (rs_sbookborrow_due.next()) 
			{
				show_sid.add(rs_sbookborrow_due.getString("sid"));
			}
			rs_sbookborrow_due.close();
			for (String sid: show_sid)
			{
				stmt.executeUpdate("UPDATE sAccount SET otmark = 1 WHERE sid = '" + sid + "'");
			}
	...
	public boolean ui_bookRequest(int bookID)
	{
		...
		try {
			...
			while (UI_bookRequest_status)
			{
				...
				{
					outPrintln("Item is reserved currently.");
					outPrintln("1. Request");
					outPrintln("0. Cancel");
					newLine();
					outPrintln(H_LINE + H_LINE + makeStars(" Book Request ".length()) + H_LINE + H_LINE);
					...
					switch (choice) 
					{
						...
						case 1 :
							outPrintln("Request!");
							newLine();
							requestResult = registerRequestReserved(bookID);
							UI_bookRequest_status = STOP;
							break;
						default:
							outPrintln("Invalid selection!");
		...
		try {
			while (UI_registerRequest_status)
			{
				...
				switch (choice)
				{
					case 1 :
						int in_sb_bid = 1;
						ResultSet rs_sbookborrow = stmt.executeQuery("SELECT * FROM sbookborrow");
						while (rs_sbookborrow.next())
						{
							in_sb_bid++; // count rows in fbookborrow
						}
						...
						rs_sbookborrow = stmt.executeQuery("SELECT * FROM sbookborrow WHERE (stime <= to_date('" + sdf.format(in_stime) + "', 'mm/dd/yy hh24:mi') AND etime > to_date('" + sdf.format(in_stime) + "', 'mm/dd/yy hh24:mi')) OR (stime <= to_date('" + sdf.format(in_etime) + "', 'mm/dd/yy hh24:mi') AND etime > to_date('" + sdf.format(in_etime) + "', 'mm/dd/yy hh24:mi'))");
						// check date availability in fbookborrow
						ResultSet rs_fbookborrow = stmt.executeQuery("SELECT * FROM fbookborrow WHERE (stime <= to_date('" + sdf.format(in_stime) + "', 'mm/dd/yy hh24:mi') AND etime > to_date('" + sdf.format(in_stime) + "', 'mm/dd/yy hh24:mi')) OR (stime <= to_date('" + sdf.format(in_etime) + "', 'mm/dd/yy hh24:mi') AND etime > to_date('" + sdf.format(in_etime) + "', 'mm/dd/yy hh24:mi'))");
						...
						if (rs_fbookborrow.next() || rs_sbookborrow.next() || (numOfBooks<1))
						{
							...
							ResultSet rs_sbookwl = stmt.executeQuery("SELECT * FROM sbookwl");
							int in_sbwl_id = 1;
							while (rs_sbookwl.next())
							{
								in_sbwl_id++;
							}
							rs_sbookwl.close();
							stmt.executeUpdate("INSERT INTO sbookwl VALUES (" + in_sbwl_id + ", '" + loginID + "', " + bookID + ")");
						...
						{
							outPrintln("Date not available!");
							outPrintln("Duration should be within 14 days, and not overlap with other dates.");
						}
						else
						{
							String in_blocation = "N/A";
							stmt.executeUpdate("INSERT INTO sBookBorrow VALUES (" + in_sb_bid + ", '" + loginID + "', " + bookID + ", '" + in_blocation + "', to_date('" + sdf.format(in_stime) + "', 'mm/dd/yy hh24:mi'), to_date('"+ sdf.format(in_etime) + "', 'mm/dd/yy hh24:mi'), " + "0)");
							numOfBooks--;
	...
	/* register request of reserved book by enrolled student */
	public boolean registerRequestReserved(int bookID)
	{
		boolean registerResult = false;
		boolean UI_registerRequest_status = RUN;
		try {
			...
			{
				newLine();
				outPrintln(H_LINE + H_LINE + " Register Request of Reserved Book " + H_LINE + H_LINE);
				outPrintln("1. Input Register Info");
				outPrintln("0. Cancel");
				newLine();
				outPrintln(H_LINE + H_LINE + makeStars(" Register Request of Reserved Book ".length()) + H_LINE + H_LINE);
				newLine();
				outPrint("Input the selection: ");
				int choice = user_input.nextInt();
				switch (choice)
				{
					case 1 :
						ResultSet rs_enrollment = stmt.executeQuery("SELECT * FROM enrollment E, bookreserve BR WHERE E.sid = '" + loginID + "' AND BR.cid = E.cid AND BR.bid = " + bookID);
						if (rs_enrollment.next())
						{
							...
							{
								outPrintln("Date not available!");
								outPrintln("Duration should be within 4 hours, and not overlap with other dates.");
							}
							else 
							{
								String in_blocation = "N/A";
						...
						{
							outPrintln("You are not enrolled in the course.");
						}
						rs_enrollment.close();
						UI_registerRequest_status = STOP;
						break;
			...
			while (UI_EbookRequest_status)
			{
				...
				switch (choice) 
				{
					...
					case 1 :
						...
						String in_blocation = "N/A";
						stmt.executeUpdate("INSERT INTO sBookBorrow VALUES (" + in_sb_bid + ", '" + loginID + "', " + bookID + ", '" + in_blocation + "', to_date('" + sdf.format(in_stime) + "', 'mm/dd/yy hh24:mi'), NULL, " + "0)");
						requestResult = true;
						UI_EbookRequest_status = STOP;
						break;
					default:
		...
		try {
			while (UI_registerJournalRequest_status)
			{
				...
				switch (choice)
				{
					case 1 : 
						int in_sj_jid = 1;
						ResultSet rs_sjournalborrow = stmt.executeQuery("SELECT * FROM sjournalborrow");
						while (rs_sjournalborrow.next())
						{
							in_sj_jid++; // count rows in fjournalborrow
						}
						rs_sjournalborrow.close();
						user_input = new Scanner(System.in);
						outPrintln("Input the checkout and return date. ");
						SimpleDateFormat sdf = new SimpleDateFormat("MM/dd/yy HH:mm");
						outPrintln("check out date(mm/dd/yy HH:mi): ");
						...
						Date in_etime = sdf.parse(user_input.nextLine());
						double diffHours = (double)(in_etime.getTime() - in_stime.getTime()) / (60.0 * 60.0 * 1000);
						// check date availability in sjournalborrow
						rs_sjournalborrow = stmt.executeQuery("SELECT * FROM sjournalborrow WHERE (stime <= to_date('" + sdf.format(in_stime) + "', 'mm/dd/yy hh24:mi') AND etime > to_date('" + sdf.format(in_stime) + "', 'mm/dd/yy hh24:mi')) OR (stime <= to_date('" + sdf.format(in_etime) + "', 'mm/dd/yy hh24:mi') AND etime > to_date('" + sdf.format(in_etime) + "', 'mm/dd/yy hh24:mi'))");
						// check date availability in fjournalborrow
						ResultSet rs_fjournalborrow = stmt.executeQuery("SELECT * FROM fjournalborrow WHERE (stime <= to_date('" + sdf.format(in_stime) + "', 'mm/dd/yy hh24:mi') AND etime > to_date('" + sdf.format(in_stime) + "', 'mm/dd/yy hh24:mi')) OR (stime <= to_date('" + sdf.format(in_etime) + "', 'mm/dd/yy hh24:mi') AND etime > to_date('" + sdf.format(in_etime) + "', 'mm/dd/yy hh24:mi'))");
						if (rs_sjournalborrow.next() || rs_fjournalborrow.next() || (diffHours > 12))
						{
							outPrintln("Date not available!");
						...
						{
							String in_blocation = "N/A";
							stmt.executeUpdate("INSERT INTO sJournalBorrow VALUES (" + in_sj_jid + ", '" + loginID + "', " + journalID + ", '" + in_blocation + "', to_date('" + sdf.format(in_stime) + "', 'mm/dd/yy hh24:mi'), to_date('"+ sdf.format(in_etime) + "', 'mm/dd/yy hh24:mi'), " + "0)");
							registerResult = true;
		...
		try {
			while (UI_registerConferenceRequest_status)
			{
				...
				switch (choice)
				{
					case 1 : 
						int in_sc_bid = 1;
						ResultSet rs_sconfborrow = stmt.executeQuery("SELECT * FROM sconfborrow");
						while (rs_sconfborrow.next())
						{
							in_sc_bid++; // count rows in sconfborrow
						}
						rs_sconfborrow.close();
						user_input = new Scanner(System.in);
						outPrintln("Input the checkout and return date. ");
						SimpleDateFormat sdf = new SimpleDateFormat("MM/dd/yy HH:mm");
						outPrintln("check out date(mm/dd/yy HH:mi): ");
						...
						Date in_etime = sdf.parse(user_input.nextLine());
						double diffHours = (double)(in_etime.getTime() - in_stime.getTime()) / (60.0 * 60.0 * 1000);
						// check date availability in sconfborrow
						rs_sconfborrow = stmt.executeQuery("SELECT * FROM sconfborrow WHERE (stime <= to_date('" + sdf.format(in_stime) + "', 'mm/dd/yy hh24:mi') AND etime > to_date('" + sdf.format(in_stime) + "', 'mm/dd/yy hh24:mi')) OR (stime <= to_date('" + sdf.format(in_etime) + "', 'mm/dd/yy hh24:mi') AND etime > to_date('" + sdf.format(in_etime) + "', 'mm/dd/yy hh24:mi'))");
						// check date availability in fconfborrow
						ResultSet rs_fconfborrow = stmt.executeQuery("SELECT * FROM fconfborrow WHERE (stime <= to_date('" + sdf.format(in_stime) + "', 'mm/dd/yy hh24:mi') AND etime > to_date('" + sdf.format(in_stime) + "', 'mm/dd/yy hh24:mi')) OR (stime <= to_date('" + sdf.format(in_etime) + "', 'mm/dd/yy hh24:mi') AND etime > to_date('" + sdf.format(in_etime) + "', 'mm/dd/yy hh24:mi'))");
						if (rs_sconfborrow.next() || rs_fconfborrow.next() || (diffHours > 12))
						{
							outPrintln("Date not available!");
							outPrintln("Duration should be within 12 hours, and not overlap with other dates.");
						}
						...
						{
							String in_blocation = "N/A";
							stmt.executeUpdate("INSERT INTO sConfBorrow VALUES (" + in_sc_bid + ", '" + loginID + "', " + conferenceID + ", '" + in_blocation + "', to_date('" + sdf.format(in_stime) + "', 'mm/dd/yy hh24:mi'), to_date('"+ sdf.format(in_etime) + "', 'mm/dd/yy hh24:mi'), " + "0)");
							registerResult = true;
	...
	{
		Project_DB_Publications_student pdb_pub_itv = new Project_DB_Publications_student("S3");
	}
}